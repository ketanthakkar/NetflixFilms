...
export const fetchMovieDetail = movieId => dispatch => fetch(`${URL}/${movieId}`)
  .then(response => response.json())
  .then((result) => {
...
export const fetchSearchedMovies = phrase => dispatch => fetch(getUrl(phrase, 'title', 'release_date'))
  .then(response => response.json())
  .then((result) => {
    dispatch(receiveMovies(result));
  });
